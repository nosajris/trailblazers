...
export const load: PageServerLoad = async () => {
	const items = await services.parents.getAllForAdmin();
	return { items };
};

export const actions: Actions = {
	saveParentContent: async ({ request, locals }) => {
		...
		const saved = await services.parents.saveParentContent({
			id,
			headline,
			subheadline,
			body,
		...
		await services.auditLogs.logAction(
			id ? 'UPDATE_PARENT_CONTENT' : 'CREATE_PARENT_CONTENT',
			'PARENTS',
			String(saved.id),
			`Saved parents portal content: ${saved.headline}`,
			locals.user?.id,
	...
	deleteParentContent: async ({ request, locals }) => {
		const form = await request.formData();
		const id = Number(form.get('id'));
		if (id) {
			await services.parents.deleteParentContent(id);
			await services.auditLogs.logAction(
				'DELETE_PARENT_CONTENT',
				'PARENTS',
				String(id),
				`Deleted parent content ID: ${id}`,
				locals.user?.id,
				locals.user?.fullName
			);
		}
